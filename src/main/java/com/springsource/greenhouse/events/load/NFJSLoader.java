...
public class NFJSLoader {
	...
	private void loadEventSessionData(int showId, long event, String abbreviation) {
		List<Map<String, Object>> topicMapList = restTemplate
				.getForObject("https://springone2gx.com/m/data/show_topics.json?showId={showId}", List.class, showId);
			...
			for (Integer speakerId : speakerIds) {
				...
				if (leaderId != null) {
					...
					if (logger.isWarnEnabled()) {
						logger.warn(String.format("Unknown speaker ID %d. Not in show_speakers.json feed.", speakerId));
					}
				}
			}
