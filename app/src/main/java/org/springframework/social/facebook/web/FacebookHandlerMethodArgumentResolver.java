...
// FacebookHandlerMethodArgumentResolverクラスは、Facebookのクッキー情報を解析し、必要なデータを取得するためのメソッドを提供します。
public class FacebookHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	// supportsParameterメソッドは、メソッドパラメータにFacebookCookieValueアノテーションが付いているかどうかを確認します。
	public boolean supportsParameter(MethodParameter parameter) {
		return parameter.getParameterAnnotation(FacebookCookieValue.class) != null;
	}

	// resolveArgumentメソッドは、リクエストからFacebookのクッキー情報を解析し、指定されたキーの値を返します。
	// FacebookCookieValueアノテーションからキーを取得し、クッキーにそのキーが存在しない場合、アノテーションが必須とされていれば例外をスローします。
	public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer modelAndViewContainer,
		...
		Map<String, String> cookieData = FacebookCookieParser.getFacebookCookieData(nativeRequest.getCookies(), appId,
				appSecret);
