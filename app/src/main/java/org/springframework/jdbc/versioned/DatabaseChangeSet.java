...
/**
 * A group of changes that should be applied atomically to upgrade the database
...
public class DatabaseChangeSet implements Comparable<DatabaseChangeSet> {
	...
	/**
	 * Apply the changes in this change set to upgrade the database to
	 * {@link #getVersion() version}.
	 * Should only be called if the version of the Database is less than the version
	 * of this change set.
	 * Only upgrade the Database version if all changes in this set are applied
	 * successfully.
	 * If any change fails, the entire set will be rolled back and no changes will
	 * be applied.
	 */
	...
	private Connection getTransactionalConnection(DataSource dataSource) {
		try {
			...
			throw new DatabaseChangeException("Could not get JDBC Connection", e);
		}
	}
	...
	private void beginTransaction(Connection connection) {
		try {
			...
			throw new DatabaseChangeException("Could not begin database transaction", e);
		}
	}
	...
	private void updateDatabaseVersion(DatabaseVersion version, Connection connection) {
		try {
			...
			throw new DatabaseChangeException("Failed to update database version", e);
		}
	}
	...
	private void commitTransaction(Connection connection) {
		try {
			...
			throw new DatabaseChangeException("Could not commit transaction", e);
		}
	}
	...
	private void rollbackTransaction(Connection connection) {
		try {
			...
			throw new DatabaseChangeException("Could not rollback transaction", e);
		}
	}
