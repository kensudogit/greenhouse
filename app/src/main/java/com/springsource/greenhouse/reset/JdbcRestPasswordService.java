...
public class JdbcRestPasswordService implements ResetPasswordService {
	...
	public boolean isValidResetToken(String token) {
		return jdbcTemplate.queryForObject("select count(*) from ResetPassword where token = ?", Integer.class, token) == 1;
	}
	...
	private Long findAccountIdByToken(String token) throws InvalidResetTokenException {
		try {
			...
			return jdbcTemplate.queryForObject("select member from ResetPassword where token = ?", Long.class, token);
