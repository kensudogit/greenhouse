...
/**
 * EventLoaderRepository implementation that loads Event data into a relational
...
public class JdbcEventLoaderRepository implements EventLoaderRepository {
	...
	public long loadEvent(EventData eventData, VenueData venueData) {
		try {
			// 外部イベントからイベントと最終更新日を取得するクエリ
			String query = "SELECT event, lastUpdated FROM ExternalEvent WHERE sourceId = ? AND source = ?";
			Map<String, Object> result = jdbcTemplate.queryForMap(query, eventData.getSourceId(),
					eventData.getSource());

			int eventId = (Integer) result.get("event");
			Date lastUpdated = (Date) result.get("lastUpdated");

			// 日付を比較して、必要に応じてイベントを更新
			if (lastUpdated.before(new Date())) {
				// Update the event if the last updated date is before the current date
				...
				logger.info("No update needed for event (ID = {})", eventId);
			}
			int venueId = jdbcTemplate.queryForObject(SELECT_EVENT_VENUE, Integer.class, eventId);
			...
			jdbcTemplate.update(new PreparedStatementCreatorFactory(INSERT_EVENT,
					new int[] { VARCHAR, VARCHAR, VARCHAR, TIMESTAMP, TIMESTAMP, VARCHAR, BIGINT })
					.newPreparedStatementCreator(new Object[] { eventData.getName(), eventData.getDescription(),
							eventData.getTimeZone(), eventData.getFirstDay(), eventData.getLastDay(),
			...
			jdbcTemplate.update(new PreparedStatementCreatorFactory(INSERT_VENUE,
					...
					.newPreparedStatementCreator(new Object[] { venueData.getName(), venueData.getPostalAddress(),
							venueData.getLatitude(), venueData.getLongitude(), venueData.getLocationHint(), 1 }),
					keyHolder);
	...
	public long loadLeader(LeaderData leaderData) {
		try {
			// 既存のリーダーを更新
			int leaderId = jdbcTemplate.queryForObject(SELECT_EXTERNAL_LEADER, Integer.class, leaderData.getSourceId(),
					leaderData.getSource());
			jdbcTemplate.update(UPDATE_LEADER, leaderData.getName(), leaderData.getBio(), leaderData.getPersonalUrl(),
					leaderData.getTwitterId(), leaderId);
			...
			KeyHolder keyHolder = new GeneratedKeyHolder();
			jdbcTemplate.update(
					new PreparedStatementCreatorFactory(INSERT_LEADER, new int[] { VARCHAR, VARCHAR, VARCHAR, VARCHAR })
	...
	public long loadTimeSlot(TimeSlotData timeSlotData) {
		try {
			// 既存のタイムスロットを更新
			int timeSlotId = jdbcTemplate.queryForObject(SELECT_EXTERNAL_TIMESLOT, Integer.class, timeSlotData.getSourceId(),
			...
			KeyHolder keyHolder = new GeneratedKeyHolder();
			jdbcTemplate.update(
					new PreparedStatementCreatorFactory(INSERT_TIMESLOT,
							new int[] { BIGINT, VARCHAR, TIMESTAMP, TIMESTAMP })
							.newPreparedStatementCreator(new Object[] { timeSlotData.getEventId(),
									timeSlotData.getLabel(), timeSlotData.getStartTime(), timeSlotData.getEndTime() }),
					keyHolder);
	...
	public int loadEventSession(EventSessionData sessionData) {
		try {
			...
			jdbcTemplate.update(UPDATE_SESSION,
					sessionData.getTitle(), sessionData.getDescription(), sessionData.getHashtag(),
					sessionData.getVenue(), sessionData.getTimeslot(), sessionKey[0], sessionKey[1]);
			logger.info("Updated session (EVENT = {}, ID = {})", sessionKey[0], sessionKey[1]);
			return 1;
		} catch (IncorrectResultSizeDataAccessException e) {
			// insert if it doesn't exist
			Integer maxId = jdbcTemplate.queryForObject("select max(id) from EventSession where event=?",
					Integer.class, sessionData.getEvent());
			int newSessionId = (maxId != null ? maxId : 0) + 1;
			...
			jdbcTemplate.update(INSERT_EXTERNAL_SESSION, sessionData.getEvent(), newSessionId, sessionData.getSource(),
					sessionData.getSourceId(), new Date());
